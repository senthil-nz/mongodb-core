...
function reconnectProxies(self, proxies, callback) {
  ...
  function execute(_server, i) {
    setTimeout(function() {
      ...
      var hostName = _server.name.split(':')[0];
      var portNumber = parseInt(_server.name.split(':')[1], 10);
      //handle IPv6 address
      if (_server.name.startsWith('[')) {
        hostName = _server.name.split(']:')[0] + "]";
        portNumber = parseInt(_server.name.split(']:')[1], 10);
        self.s.options.family = 6;
      }
